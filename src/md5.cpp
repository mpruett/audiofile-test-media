 ...
 */
void MD5Final(MD5Digest *digest, MD5Context *ctx)
{
	...
	*p++ = 0x80;
	...
	// Compute bytes of padding needed to make 64 bytes.
	count = 64 - 1 - count;
...
{
	static const char toBase16[] = "0123456789abcdef";
	std::string r;
	r.resize(32);

	for (int i=0; i<16; i++)
	{
		int v = digest.digest[i];
		r[2*i] = toBase16[(v>>4) & 0xf];
		r[2*i+1] = toBase16[v & 0xf];
	}
	return r;
}
